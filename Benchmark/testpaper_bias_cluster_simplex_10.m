...
function testpaper_bias_cluster_simplex_10(idx)
...
for c = [idx]
    ...
    for dim = dimInput
        ...
        opts.logging = 'testpaper_bias_benchmark.log'; % Output the debug log to testpaper_benchmark.log
        
        for i=1:nb_repet
            opts.seed = randi(1024)+1+nb_repet;
            o = sample_modif(P, numSamples, opts);
            ...
            save(fullfile(curFolder, strcat('bias_analysis_local/','_', inst, int2str(dim),'_rep_',num2str(i)))        , 'exps');
        end
end
end
