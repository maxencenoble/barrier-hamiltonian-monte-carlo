    ...
    methods
        ...
        function o = refresh_bar(o, s)
            ...
            if (s.N == Inf)
                ...
                str2 = sprintf(fmt, durationString(timeSpent), durationString(timeRemain), progressString(progress, o.barLength), nSamples, prob, min(s.stepSizeAdap), avgMixingTime);
            else
                fmt = sprintf('%s%i%s%i%s', '%12s | %12s | %s | %', o.nSamplesTextLength, 'i / %', o.nSamplesTextLength, 'i | %8.6f | %8.6f | %8.1f');
                str2 = sprintf(fmt, durationString(timeSpent), durationString(timeRemain), progressString(progress, o.barLength), nSamples, s.N, prob, min(s.stepSizeAdap), avgMixingTime);
            end
