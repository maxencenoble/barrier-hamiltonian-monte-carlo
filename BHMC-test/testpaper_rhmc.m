...
for gamma0 = gamma0_list
      try
        ...
        opts.seed = 1;
        opts.logging = strcat('testpaper_rhmc_original', num2str(idx_pb), '.log'); % Output the debug log to testpaper_rhmc.log
        ...
        save(fullfile(curFolder, strcat('/rhmc_test/result_rhmc_original_', filename, matfiles(c).name)), 'exps')
        clear('exps');
        clear('o');
    catch e
